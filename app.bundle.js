...
// this wrapper and call unlockAudioOnce() (e.g. voice input toggle). Without
// exporting to window, those calls throw "ReferenceError: unlockAudioOnce is not defined".
global.unlockAudioOnce = unlockAudioOnce;
global.playAssistantAudio = playAssistantAudio;
...
let pendingCheckoutInterval = null;
...
// If not signed in, keep this as a single-step flow: create account (Email/Google) then go straight to Stripe.
if (!currentUser) {
  const startTrialAfterAuth = (interval) => {
    try { pendingCheckoutInterval = interval; } catch (_) {}
  };

  const googleBtn = _onbBtn('Continue with Google & Start Trial', { kind: 'primary', onClick: () => {
    startTrialAfterAuth('monthly');
    // Start Netlify Identity Google OAuth. Requires Netlify Identity + Google provider configured.
    const returnTo = encodeURIComponent(window.location.href);
    window.location.href = `/.netlify/identity/login/google?returnTo=${returnTo}`;
  }});
  actions.appendChild(googleBtn);

  const emailBtn = _onbBtn('Continue with Email & Start Trial', { kind: 'secondary', onClick: () => {
    startTrialAfterAuth('monthly');
    openIdentityModal('signup');
  }});
  ...
  fineprint.innerText = 'Start your free trial after creating an account. You can cancel anytime.';
  return;
}

// Signed in: allow direct checkout.
...
async function loadGoal() {
  ...
  try { localStorage.setItem('calorie_goal_base', String(j.daily_calories ?? 0));
    const __effective = (j && typeof j.effective_daily_calories === 'number') ? j.effective_daily_calories : (j.daily_calories ?? 0);
    localStorage.setItem('calorie_goal', String(__effective));
    localStorage.setItem('rollover_delta', String((j && typeof j.rollover_delta==='number') ? j.rollover_delta : 0)); } catch(e) {}
...
(function() {
  ...
  // Coach voice mode (stored locally). When enabled, FAB starts voice capture instead of opening the textbox UI.
  ...
  function setCoachListeningOverlay(isOn) {
    ...
    if (overlay) overlay.classList.toggle('hidden', !isOn);
    if (pill) pill.classList.toggle('hidden', !isOn);
  }
    ...
    coachFab.onclick = async () => {
      if (getCoachVoiceMode()) {
        ...
        try {
          ...
          setCoachListeningOverlay(false);
          if (text) {
            // send to coach without forcing focus on textbox
            const card = el('coachChatCard');
            if (card) { card.classList.add('open'); coachFab.setAttribute('aria-expanded','true'); }
            await sendChat({ from_voice: true, message: text });
          }
        } catch (e) {
          setCoachListeningOverlay(false);
          setStatus(e.message || String(e));
      ...
      // default: toggle coach chat open/closed
      const card = el('coachChatCard');
      if (!card) return;
...
if (typeof netlifyIdentity !== 'undefined') {
  netlifyIdentity.on('init', user => {
    ...
    .then(() => {
      try {
        const pending = pendingCheckoutInterval;
        pendingCheckoutInterval = null;
        if (pending && billingController) billingController.startUpgradeCheckout(pending);
      } catch (_) {}
    })
...
function getBaseDailyCalorieGoal() {
  ...
  const base = parseInt(localStorage.getItem('calorie_goal_base') || localStorage.getItem('calorie_goal') || '0', 10) || 0;
  return base;
}
...
function initCoachVoiceModeSettings() {
  const t = document.getElementById('coachVoiceModeToggle');
  const s = document.getElementById('coachVoiceModeStatus');
  if (!t) return;
...
document.addEventListener("DOMContentLoaded", ()=>{
  ...
  try { initRolloverCaloriesSettings(); } catch(e) {}
  try { initCoachVoiceModeSettings(); } catch(e) {}
});
