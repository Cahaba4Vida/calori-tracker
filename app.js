...
async function uploadUnifiedPhotoFromInput(inputId = 'photoUnifiedInput', mode = null) {
  ...
  try {
    ...
    let j;
    try {
      j = await withThinking(async () => api('entries-add-image', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ imageDataUrl, extract_only: true })
      }));
      ...
      // If nutrition-label extraction fails (e.g., this is a plate photo), fallback to plate estimate.
      console.warn('Label scan failed; falling back to plate estimate.', eLabel);
      setStatus('Estimating plate…');
      const est = await withThinking(async () => api('entries-estimate-plate-image', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ imageDataUrl, servings_eaten: 1.0, portion_hint: null })
      }));
      ...
      pendingPlateEstimate = est;
      el('estimateServingsInput').value = '1';
      el('estimateCaloriesInput').value = est.calories ?? '';
      el('estimateProteinInput').value = (est.protein_g == null ? '' : String(Math.round(est.protein_g)));
      el('estimateCarbsInput').value = (est.carbs_g == null ? '' : String(Math.round(est.carbs_g)));
      el('estimateFatInput').value = (est.fat_g == null ? '' : String(Math.round(est.fat_g)));
      setBadge(est.confidence || 'low');

      const ul = el('estimateAssumptions');
      if (ul) {
        ul.innerHTML = '';
        const list = (est.assumptions && est.assumptions.length) ? est.assumptions : (est.items || []);
        list.forEach((txt) => {
          const li = document.createElement('li');
          li.textContent = txt;
          ul.appendChild(li);
      ...
      const notes = el('estimateNotes');
      if (notes) notes.innerText = est.notes ? est.notes : '';
